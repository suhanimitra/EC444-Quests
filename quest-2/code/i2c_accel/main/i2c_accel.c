//Leyandra Burke, Suhani Mitra, Margherita Piana, Kyla Wilson  10/04/2024
...
static int classifyActivity(float x, float y, float z, float roll, float pitch, int *activTimeS){
  ...
  static int lastState;
    ...
    else if ((roll > -1 && roll < 1) && (pitch > -1 && pitch < 1)) {
      state = 0;  //asleep
...
static void display_info(int* activTimeS){
  if (button_on == 1) {
    ...
    input[4]=' ';
    input[4]=' ';
    input[5]=activity_string[0];
    input[6]=activity_string[1];
    input[7]=activity_string[2];
    input[8]=activity_string[3];
    input[9]=activity_string[4];
    input[10]=activity_Time[0];
    input[11]=activity_Time[1];
    input[12]=activity_Time[2];
    input[13]= ' ';
    input[14]= 'S';
  ...
  for (int offset = 0; offset <= input_len - 4; offset++) {
            // Write the next 4 characters to the display
            uint16_t displaybuffer[4];
            for (int i = 0; i < 4; i++) {
                displaybuffer[i] = encode_char_to_segment(input[offset + i]);
    ...
    printf("Test: %d Time: %d Temperature: %f CurrentTime: %02d:%02d:%02d\n",state,*activTimeS,temp,hours,minutes,seconds);
    //*activTimeS++;
...
static void test_adxl343() {
  ...
  while (1) {
    ...
    //printf("Activity state: %d,Button stete %d, Temperature %f \n",state,button_on, temp);
